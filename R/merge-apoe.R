apoe <- function(dat){
    ...
    CountAlleles <- function(string, allele){
        ifelse(is.na(string), NA, 
            ifelse(string %in% c("Undetermined"), NA,
    ...
    dat <- within(dat, {
        ...
        apoe4pos.factor <- factor(apoe4pos,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(apoe4pos) <- label(apoe4pos.factor) <-
        ...
        apoe2pos.factor <- factor(apoe2pos,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(apoe2pos) <- label(apoe2pos.factor) <-
            'ApoE2+ (at least one E2 allele)'
