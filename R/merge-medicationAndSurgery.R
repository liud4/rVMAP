# 09 Jul 2016, LS:
## making medFrame regex more robust

# 21-28 Sep 2016, LS:
## adding processing for antihypertensive-med subtypes

# 07 Oct 2016, LS:
## Changing definition of htnrx to align w/ the new subtypes

medicationAndSurgery <- function(dat, diabFile, cholFile, 
    #antihypFile, 
    ...
    diabNums <- read.csv(diabFile)[, 1]
    cholNums <- read.csv(cholFile)[, 1]
    afibNums <- read.csv(afibFile)[, 1]
    afibsurgNums <- read.csv(afibsurgFile)[, 1]

    # anti-hyp. subtype files
    ahBetaBlockerNums <- read.csv(antihypBetaBlockerFile)[, 1]
    ahBetaBlockerIfNotDropNums <- 
        read.csv(antihypBetaBlockerIfNotDropFile)[, 1]
    ahACEInhibNums <- read.csv(antihypACEInhibFile)[, 1]
    ahARBNums <- read.csv(antihypARBFile)[, 1]
    ahCCBNums <- read.csv(antihypCCBFile)[, 1]
    ahKSDNums <- read.csv(antihypKSDFile)[, 1]
    ahOtherNums <- read.csv(antihypOtherFile)[, 1]

    # 07 Oct 2016: now a med is antihyp only if it's in one of the subtypes
    #antihypNums <- read.csv(antihypFile)[, 1]
    ...
    for (i in 1:nrow(dat)) {
        possibleEyedropCols <- which(medFrame[i, ] %in% 
            ahBetaBlockerIfNotDropNums)
        if (length(possibleEyedropCols) >= 1) {
            mednames <- names(medFrame)[possibleEyedropCols]
            unitnames <- gsub("name", "units.factor", mednames,
                fixed= TRUE)
            vec <- dat[i, unitnames]
    ...
    ahBetaBlockerPrep <- 
        ifelse(ahBetaBlockerPrep0 %in% 1, 1,
        ifelse(ahBetaBlockerIfNotDropPrep %in% 1, 1,
        ifelse(is.na(ahBetaBlockerIfNotDropPrep), NA, 0)))
    ...
    dat <- within(dat, {
        ...
        diabetesrx.factor <- factor(diabetesrx,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(diabetesrx) <- label(diabetesrx.factor) <-
            'Taking at least 1 diabetes med'

        cholesterolrx <- ifelse(is.na(meds), NA, cholesterolrxPrep)
        cholesterolrx.factor <- factor(cholesterolrx,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(cholesterolrx) <- label(cholesterolrx.factor) <-
            'Taking at least 1 cholesterol med'

        afibrx <- ifelse(is.na(meds), NA, afibrxPrep)
        afibrx.factor <- factor(afibrx,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(afibrx) <- label(afibrx.factor) <-
            'Taking at least 1 afib med'

        afibsurg <- ifelse(is.na(surg01), NA, afibsurgPrep)
        afibsurg.factor <- factor(afibsurg,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(afibsurg) <- label(afibsurg.factor) <-
            'At least 1 afib surgery'

        rx.htn.beta.blocker <- 
            ifelse(is.na(meds), NA, ahBetaBlockerPrep)
        rx.htn.beta.blocker.factor <- factor(rx.htn.beta.blocker,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(rx.htn.beta.blocker) <- label(rx.htn.beta.blocker.factor) <-
            'Taking at least 1 beta-blocker'

        rx.htn.ace.inhibit <- 
            ifelse(is.na(meds), NA, ahACEInhibPrep)
        rx.htn.ace.inhibit.factor <- factor(rx.htn.ace.inhibit,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(rx.htn.ace.inhibit) <- label(rx.htn.ace.inhibit.factor) <-
            'Taking at least 1 ACE inhibitor'

        rx.htn.arb <- 
            ifelse(is.na(meds), NA, ahARBPrep)
        rx.htn.arb.factor <- factor(rx.htn.arb,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(rx.htn.arb) <- label(rx.htn.arb.factor) <-
            'Taking at least 1 ARB'

        rx.htn.ccb <- 
            ifelse(is.na(meds), NA, ahCCBPrep)
        rx.htn.ccb.factor <- factor(rx.htn.ccb,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(rx.htn.ccb) <- label(rx.htn.ccb.factor) <-
            'Taking at least 1 Ca channel blocker'

        rx.htn.ksd <- 
            ifelse(is.na(meds), NA, ahKSDPrep)
        rx.htn.ksd.factor <- factor(rx.htn.ksd,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(rx.htn.ksd) <- label(rx.htn.ksd.factor) <-
            'Taking at least 1 K-sparing diuretic'

        rx.htn.other <- 
            ifelse(is.na(meds), NA, ahOtherPrep)
        rx.htn.other.factor <- factor(rx.htn.other,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(rx.htn.other) <- label(rx.htn.other.factor) <-
            "Taking at least 1 antihyp. med of type 'Other'"
    })

    ahSubtypeDat <- dat[, Cs(rx.htn.beta.blocker, rx.htn.ace.inhibit, rx.htn.arb, rx.htn.ccb, rx.htn.ksd, rx.htn.other)]
