...
biomarkers <- function(dat){
    ...
    dat <- within(dat, {
        ...
        amyloidPos <- ifelse(is.na(csf.ab1.42), NA,
            ifelse(csf.ab1.42 < 530, 1, 0))
        amyloidPos.factor <- factor(amyloidPos,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(amyloidPos) <- label(amyloidPos.factor) <-
            "Amyloid positive (csf.ab1.42 < 530 ng/L)"

        tauPos <- ifelse(is.na(csf.tau), NA,
            #ifelse(csf.tau > 350, 1, 0))
            ifelse(csf.tau > 400, 1, 0))
        tauPos.factor <- factor(tauPos,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(tauPos) <- label(tauPos.factor) <-
            #"Tau positive (csf.Tau > 350 ng/L)"
            "Tau positive (csf.Tau > 400 ng/L)"

        ptauPos <- ifelse(is.na(csf.ptau), NA,
            #ifelse(csf.ptau > 60, 1, 0))
            ifelse(csf.ptau > 80, 1, 0))
        ptauPos.factor <- factor(ptauPos,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(ptauPos) <- label(ptauPos.factor) <-
            #"P-Tau positive (csf.ptau > 60 ng/L)"
            "P-Tau positive (csf.ptau > 80 ng/L)"
        
        biomarkers.vegf <- ifelse(biomarkers.vegf=="<11.32_",11.32,
                                  ifelse(biomarkers.vegf=="<10.61_",10.61,
                                         ifelse(biomarkers.vegf=="<9.56_",9.56,
                                                ifelse(biomarkers.vegf=="<13.51_",13.51,
                                                       ifelse(biomarkers.vegf=="<13.51?",13.51,biomarkers.vegf)))))

        biomarkers.il6 <- ifelse(biomarkers.il6=="<0.33_",0.33,
                                 ifelse(biomarkers.il6=="<0.66_",0.66,biomarkers.il6))
        ...
        # csf.snap groups
        csf.snap <- 
          ifelse( is.na(amyloidPos.factor) | is.na(tauPos.factor), NA,
                  ifelse(amyloidPos.factor == "No" &  tauPos.factor== "No", 1,
                         ifelse(amyloidPos.factor == "Yes" & tauPos.factor== "No", 2,
                                ifelse(amyloidPos.factor == "No" &  tauPos.factor== "Yes", 3,
                                       ifelse(amyloidPos.factor == "Yes" & tauPos.factor== "Yes", 4, NA)))))
        
        csf.snap.factor <- factor(csf.snap,        
                                  levels= 1:4, labels= c("AB-/tau-",
                                                         "AB+/tau-",
                                                         "AB-/tau+",
                                                         "AB+/tau+"),
                                  ordered=TRUE
