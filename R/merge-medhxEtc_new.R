...
# 11 May 2015, LS:
## Replaced bld.c.insulin.adjusted with bld.c.insulin to work with JN's new code

# 05 Oct 2015, JN:
## Added additional echo.lvh variable per AJ's email request

# 31 Dec 2015, LS:
## Changing LVH definitions again to match final version of methods paper

# 25 January 2016, JN:
## Added additional variables: abp completion, lumbar puncture completion, afib during cmr

# 21 Apr 2016, LS:
## Updated diabetes definition at Angela's request

# 27 April 2016, JN
## Updated diabetes coding due to missingness

# 01 Sep 2016, JN
## Added pulse pressure (pp), anemic as derived variables

# 20 Oct 2016, LS
## Adding smoking-related derived variables

# 27 Feb 2017, JN
## Added combined afib/cvd variable which we use for secondary analyses
...
# 19 Dec 2017, DL
## revised afib code with epoch=1 using cmr and epoch>1 without using cmr
...
medhxEtc <- function(dat){
    ...
    dat <- within(dat, {
        ...
        medhx.date.year <- year(medhx.date)
    ...
    dat <- within(dat, {
        ...
        dyslipidemia.factor <- factor(dyslipidemia,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(dyslipidemia) <- label(dyslipidemia.factor) <-
            "Dyslipidemic, determined by chol, hdlc, and/or rx"

        hypertensive <- ifelse(is.na(sbp) & is.na(dbp), NA,
            ifelse(sbp >= 140 | dbp >= 90, 1,
            ifelse(sbp <  140 & dbp <  90, 0, NA)))
        hypertensive.factor <- factor(hypertensive,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(hypertensive.factor) <- label(hypertensive) <- 
            "Hypertensive, determined solely by echo SBP and/or DBP"

        htn <- ifelse(is.na(hypertensive) & is.na(htnrx), NA,
            ifelse(hypertensive == 1 | htnrx == 1, 1,
            ifelse(hypertensive == 0 & htnrx == 0, 0, NA)))
        htn.factor <- factor(htn,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(htn.factor) <- label(htn) <- 
            "Hypertensive, determined by echo BP and/or med use"
        ...
        afibecho.factor <- factor(afibecho,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(afibecho.factor) <- label(afibecho) <- 
            "Echo.rhythm = 2 or 3"

        # another convenience var for use in later calculations
        afibcmr <- ifelse(is.na(cmr.findings.rhythm), NA,
            ifelse(str_trim(tolower(cmr.findings.rhythm)) %in% 
                cmr.findings.rhythm.afib, 1,
            ifelse(str_trim(tolower(cmr.findings.rhythm)) %in% 
                cmr.findings.rhythm.normal, 0, NA)))
        afibcmr.factor <- factor(afibcmr,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(afibcmr.factor) <- label(afibcmr) <- 
            "cmr.findings.rhythm indicates afib"

        #Based on VMAC meeting on 12/19/2017, we decided to revise the code so that 
        #for epoch=1, afibconfirm is derived with cmr information 
        #for epoch>1, afibconfirm is derived without cmr information        
        afibconfirm <-ifelse(
          #########################################################
          # epoch=1 regular coding starts here for 
          #########################################################
          epoch==1,  
            ...
            ifelse(mhx.afib == 1 & # redundant but safe
                  ...
                  afibsurg  == 0 &
                  afibecho  == 0 ), 0, NA)))))            
        afibconfirm.factor <- factor(afibconfirm,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(afibconfirm.factor) <- label(afibconfirm) <- 
            "Afib self-report confirmed (NA if self-rep.= no)"

        #Based on VMAC meeting on 12/19/2017, we decided to revise the code so that 
          #for epoch=1, afib is derived using cmr information (using afibcomfirm and afibcmr)
          #for epoch>1, afib is derived without cmr information
        afib <- ifelse(
          #########################################################
          # epoch=1 regular coding starts here for 
          #########################################################
          epoch==1,
        # use afibcmr and afibconfirm (which includes afibcmr) for epoch 1 only; 
                       ifelse(is.na(mhx.afib) & is.na(afibecho) & is.na(afibcmr) , NA,
                              ifelse(
        ...
        #But it's clearer to me to do it this way.
                                      (mhx.afib == 1 & afibconfirm == 1)|
                                                          afibecho == 1 |
                                                             afibcmr == 1, 1,
                                      ifelse( mhx.afib == 0 & # implied here: is.na(afibconfirm)
                                              afibecho == 0 &
                                              afibcmr  == 0, 0, NA))),
        #########################################################
        # epoch>1 regular coding starts here for 
        #########################################################
                       ifelse(is.na(mhx.afib) & is.na(afibecho)  , NA,
                              ifelse( 
        #in next line, note that if afibconfirm == 1, it must be true
        #that mhx.afib == 1, so we don't need mhx.afib in there.
        #But it's clearer to me to do it this way.
        ...
        diabetesa1c.factor <- factor(diabetesa1c,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(diabetesa1c) <- label(diabetesa1c.factor) <-
            'bld.c.hgba1c >= 6.5'
        ...
        diabetesglucose.factor <- factor(diabetesglucose,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(diabetesglucose) <- label(diabetesglucose.factor) <-
            'bld.c.glucose >= 126'
        ...
        diabetes.factor <- factor(diabetes,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(diabetes) <- label(diabetes.factor) <-
            "Diabetic, determined by a1c, glucose, and/or rx"

        # QUICKI: 1/(log(fasting insulin) + log(fasting glucose))
        # where log is log base 10--- confirmed by Liz, 13 Oct 2014
        quicki <- 1 / (log(bld.c.insulin, base= 10) + 
            log(bld.c.glucose, base= 10))
        ...
        insulinresistance.factor <- factor(insulinresistance,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(insulinresistance.factor) <- label(insulinresistance) <- 
            "Insulin resistant as determined by QUICKI cutoff"

        # Cardiac output var. request from Angela, 28 Oct 2014
        echo.vd.mr.ge1 <- ifelse(is.na(echo.vd.mr), NA,
            ifelse(echo.vd.mr >= 1, 1, 0))
        echo.vd.mr.ge1.factor <- factor(echo.vd.mr.ge1,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(echo.vd.mr.ge1) <- label(echo.vd.mr.ge1.factor) <- 
            'Mitral valve regurgitation: echo.vd.mr >= 1'

        echo.vd.ar.ge1 <- ifelse(is.na(echo.vd.ar), NA,
            ifelse(echo.vd.ar >= 1, 1, 0))
        echo.vd.ar.ge1.factor <- factor(echo.vd.ar.ge1,
            levels= c(1, 0), labels= c("Yes", "No"))
        ...
        echo.lvmass <- round(0.8 *
            1.04 * ((echo.lvidd + echo.lvpwd + echo.ivsd)^3 - echo.lvidd^3) + 
            0.6, 2)
        ...
        cardiac.index.low.factor <- factor(cardiac.index.low, 
            levels= c(1, 0), labels= c("Yes", "No"))
        label(cardiac.index.low) <- label(cardiac.index.low.factor) <-
        ...
        echo.lvh <- 
            ...
            1))))
        echo.lvh.factor <- factor(echo.lvh,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(echo.lvh.factor) <- label(echo.lvh) <- 
            'LV hypertrophy, determined by sex and scaled LV mass'
        ...
        echo.lvh.geometry <- 
            ...
            ifelse(echo.rwt >= 0.42 | echo.lvh == 1, 1, 0))
        echo.lvh.geometry.factor <- factor(echo.lvh,
            levels= c(1, 0), labels= c("Yes", "No"))
        label(echo.lvh.geometry) <- label(echo.lvh.geometry.factor) <-
            'LV hypertrophy, determined by rel. wall thickness and/or sex and scaled LV mass'
        ...
        echo.lvh.scaledlvmass <- 
            ...
            0)))
        echo.lvh.scaledlvmass.factor <- factor(echo.lvh.scaledlvmass,
            levels=c(0,1), labels=c("No","Yes"))
        label(echo.lvh.scaledlvmass) <- label(echo.lvh.scaledlvmass.factor) <-
            'LV hypertrophy, determined by sex and scaled LV mass'
