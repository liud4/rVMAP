depression <- function(dat){
    ...
    #QIDS
    qids.sub <- paste0("qids", 
        formatC(1:16, width= 2, format= "d", flag= "0"))
    my.pmax <- function(x){
        apply(x, MARGIN= 1, FUN= function(y) 
            ifelse(sum(!is.na(y)) == 0, NA, max(y, na.rm= TRUE)))
    }
    dat$qids <- rowSums(cbind(my.pmax(dat[, qids.sub[1:4]]),
                              my.pmax(dat[, qids.sub[6:9]]),
                              my.pmax(dat[, qids.sub[15:16]]),
                              dat[, qids.sub[c(5, 10:14)]]), na.rm= TRUE)
