# create a function called blood.conversion
# input - bloodwork name, sex, age, x
# need slopes and intercepts for the line connecting low,low and high,high

# Variables that do not need to be converted:
# Potassium, cholesterol, trig, ldlc

# 30 April 2015 
# Created one bloodwork function to do all conversion
# Email Dandan and double check methods - IgM, IgA is giving negative values in conversions

# 07 May 2015 
# Renamed bloodwork variables for consistency

# 11 May 2015
# Added to bloodwork labels and moved in code from labwork.R
# Renamed file to match function name

# 12 Jun 2015
# Added creatinine clearance variable

# 9 July 2015
# Added numerical conversion for IGG

# 14 July 2015
# Added recalculated variables for bloodwork 
# Added converted variables for CSF

# 15 July 2015
# Added code for csf.c.insulin

# 20 July 2015
# Added code to deal with glucose

# 08 Jul 2016, LS:
#  explicitly returning dat.

# 11 Oct 2016, LS:
#  Renaming bld.c.igm to follow pattern of other vars (email from DL)

# 05 Dec 2016, LS:
#  renaming function to match filename, which had been changed.

# 07 Dec 2016, LS:
#   Now that we have added Epoch 2 in, the calculated values have changed
#    for some variables. The problem is caused by the code "map.id[1:91]"---
#    this is not a stable way to refer to the map id's. I'm
#    changing the code

# 13 Dec 2016, LS:
## Adding processing for bld.c.alt ("<6" in epoch 2),
#     bld.c.igg (">80.0" in epoch 2),
#     bld.c.tsh ("<0.015" in epoch 2)
