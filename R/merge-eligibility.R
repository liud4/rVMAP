...
eligibility <- function(eldat){
    # Returns eldat (should be eligibility data),
    ...
    numericCols <- names(eldat)[sapply(eldat, is.numeric)]
    eldat[, numericCols] <- apply(eldat[, numericCols], 2,
        minus9999)
    eldat[, numericCols] <- apply(eldat[, numericCols], 2,
        minus8888)
    ...
    eldat$faq.redcap <- eldat$faq
    faqvars <- paste0("faq", formatC(1:10, width= 2, flag= "0"))
    # 02 Mar 2015 (meeting): AJ and KG want to check IDs w/ any FAQ
    # items missing
    eldat$faq <- apply(eldat[, faqvars], 1, totscore, threshold= 1)

    eldat$np.srt.immed.redcap <- eldat$np.srt.immed
    np.srt.immedvars <- paste0("np.srt", 1:6)
    eldat$np.srt.immed <- apply(eldat[, np.srt.immedvars], 1, totscore, threshold= 1)

    eldat$np.cfl.redcap <- eldat$np.cfl
    np.cflvars <- c(
        paste0("np.cfl.fq", 1:4),
        paste0("np.cfl.cq", 1:4),
        paste0("np.cfl.lq", 1:4)
    )
    eldat$np.cfl <- apply(eldat[, np.cflvars], 1, totscore, threshold= 1)

    eldat$np.veg.redcap <- eldat$np.veg
    np.vegvars <- paste0("np.vegq", 1:4)
    eldat$np.veg <- apply(eldat[, np.vegvars], 1, totscore, threshold= 1)

    eldat$np.bvrt.redcap <- eldat$np.bvrt
    np.bvrtvars <- paste0("np.bvrt", formatC(1:10, width= 2, flag= "0"))
    eldat$np.bvrt <- apply(eldat[, np.bvrtvars], 1, totscore, threshold= 1)
    
    # 21 July 2015: Truncate tmta and tmtb in eligibility
    eldat$np.tmta.trun <- ifelse(eldat$np.tmta > 150, 150, eldat$np.tmta)
    eldat$np.tmtb.trun <- ifelse(eldat$np.tmtb > 240, 240, eldat$np.tmtb)
    ...
    eldatToKeep <- eldat[, c("map.id", elvarsToKeep)]
    names(eldatToKeep) <- c("map.id", paste0(elvarsToKeep, ".elig"))
