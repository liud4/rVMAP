# create project (for initial setup)
# keyring::keyring_create("VMAP_datafreeze")

# add token to each username
# keyring::key_set("REDCap_VMAP", "token_vmap_edc")
# keyring::key_set("REDCap_VMAP", "token_vmap_questionare")
# keyring::key_set("REDCap_VMAP", "token_vmap_biomarker")
# keyring::key_set("REDCap_VMAP", "token_vmap_apoe")
# keyring::key_set("REDCap_VMAP", "token_vmap_tracking")

###

data_freeze <-
  function(onedrive.dir = file.path("..", "OneDrive - VUMC"),
           output.dir = NULL,
           redcap.api.uri = "https://redcap.vumc.org/api/",
           save = TRUE,
           return = TRUE) {
    ...
    MAPfreeze.initial <- list(
      vmap_edc = list(
        project = "VMAP Electronic Data Capture Epochs 1-6",
        key = list(service = "REDCap_VMAP", username = "token_vmap_edc")
      ),
      questionnaires = list(
        project = "VMAP Questionnaires",
        key = list(service = "REDCap_VMAP", username = "token_vmap_questionare")
      ),
      biomarkers = list(
        project = "VMAP CSF, Plasma, and Serum Biomarkers",
        key = list(service = "REDCap_VMAP", username = "token_vmap_biomarker")
      ),
      apoe = list(
        project = "MAP APOE",
        key = list(service = "REDCap_VMAP", username = "token_vmap_apoe")
      ),
      tracking = list(
        project = "VMAP Participant Tracking",
        key = list(service = "REDCap_VMAP", username = "token_vmap_tracking")
      )
    )
    ...
    # unlock project
    keyring::keyring_unlock("VMAP_datafreeze")
    
    ###
    
    # use REDCap API to download data
    for (redcap.i in setdiff(names(MAPfreeze.initial), "general")) {
      ...
      MAPfreeze.list[[redcap.i]][["data"]] <- REDCapR::redcap_read(
        ...
        token = keyring::key_get(MAPfreeze.initial[[redcap.i]]$key$service, MAPfreeze.initial[[redcap.i]]$key$username),
        raw_or_label = "raw",
        verbose = TRUE
      )$data
      
      MAPfreeze.list[[redcap.i]][["metadata"]] <-
        REDCapR::redcap_metadata_read(redcap_uri = redcap.api.uri,
                                      token =  keyring::key_get(MAPfreeze.initial[[redcap.i]]$key$service, MAPfreeze.initial[[redcap.i]]$key$username),
                                      verbose = TRUE)$data
    }
    ...
    med.file.date <- 20241126 # date for main medication files
    antihypSubtype.file.date <-
      20241126 # date for antihyp subtype files
    surg.file.date <- 20230227 # date for afibsurgery
    
    # Biospecimen Availability
    biospecimen_availability.file <- file.path(data.raw.dir,
                                               "old",
                                               "VMAP_biospecimens_collected_20210719.xlsx")
    
    # Physlog data
    physlog.file <- file.path(data.raw.dir,
                              "20220916 - vmap_physlog_data.csv")
    
    # ABP consent info
    track.file <- file.path(data.raw.dir,
                            "old",
                            "MAPParticipantTracki_DATA_2015-08-26_1412.csv")
    
    # Selected data from eligibility.
    elig.file <- file.path(data.raw.dir,
                           "old",
                           "MAPEligibility_DATA_2016-11-03_1532.csv")
    
    chol.file <- file.path(med.dir,
                           paste0("CholesterolMedNumbers_", med.file.date, ".csv"))
    
    afib.file <- file.path(med.dir,
    ...
    antihypKSD.file <- file.path(
      med.dir,
      paste0(
        "antiHyp_PotassiumSparingDiuretics_",
        antihypSubtype.file.date,
        ".csv"
      )
    )
    
    antihypOther.file <- file.path(med.dir,
                                   paste0("antiHyp_Other_", antihypSubtype.file.date, ".csv"))
    
    # surgery lookup files
    afibsurg.file <- file.path(surg.dir,
                               paste0("AFibSurgNumbers_", surg.file.date, ".csv"))
    
    thyroidsurg.file <- file.path(surg.dir,
                                  paste0("ThyroidSurgNumbers_", surg.file.date, ".csv"))
    
    # epoch_1
    abp.raw.derived.file <- file.path(data.dir,
                                      "ABP_RawAndDerived",
                                      "MapAbpData_Epoch1_20150625.rds")
    
    srt.file <- file.path(data.raw.dir,
                          "old",
                          "MAPSRTErrorAnalysisEpoch1_DATA_2017-09-01_0936.csv")
    
    np.composite.file <- file.path(data.raw.dir, "np composite", "np_composite_20220926.rds")
    
    polygenetic.file <- file.path(data.raw.dir, "old",
                                  "VMAP_PRS.rds")
    
    map_163_invalidation.file <- file.path(data.dir,
                                           "Documentation",
    ...
    for (list.names in names(static.list.0)) {
      MAPfreeze.list[["static"]][[list.names]] <-
        c(MAPfreeze.list[["static"]][[list.names]], static.list.0[[list.names]])
    }
    ...
    for (list.names in names(static.list.1)) {
      MAPfreeze.list[["static"]][[list.names]] <-
        c(MAPfreeze.list[["static"]][[list.names]], static.list.1[[list.names]])
    }
    
    if (save == TRUE) {
      if (is.null(output.dir)) {
        saveRDS(object = MAPfreeze.list,
                file = file.path(data.raw.dir,
                                 paste0(
    ...
    # lock project
    keyring::keyring_lock("VMAP_datafreeze")
  }
