...
derive_blood <- function(data) {
  ...
  label(data$bld.c.insulin) = "Set to 1.9 if `<2.0', Converted Insulin Values (mcU/mL)"
  label(data$bld.c.glucose) = "Converted Glucose Level (mg/dL)"
  label(data$bld.c.sodium) = "Converted Sodium Level (mmol/L)"
  label(data$bld.c.chloride) = "Converted Chloride Level (mmol/L)"
  label(data$bld.c.co2) = "Converted Carbon Dioxide Level (mmol/L)"
  label(data$bld.c.urea.nitrogen) = "Converted BUN (mg/dL)"
  label(data$bld.c.calcium) = "Converted Calcium Level Total (mg/dL)"
  label(data$bld.c.protein.t) = "Converted Protein Level (g/dL)"
  label(data$bld.c.bilirubin.t) = "Converted Bilirubin Total (mg/dL)"
  label(data$bld.c.ast) = "Converted AST (U/L)"
  label(data$bld.c.alt) = "Set to 6 if `<6', Converted ALT (U/L)"
  label(data$bld.c.tsh) = "Set to 0.014 if `<0.015', Converted TSH (mcU/mL)"
  label(data$bld.c.creatinine) = "Converted Creatinine (mg/dL)"
  label(data$bld.c.alkphos) = "Converted Alkaline Phosphatase (U/L)"
  label(data$bld.c.crp) = "Set to 0.1 if `<0.2', Converted C-Reactive Protein HS - Cardiac (mg/L)"
  label(data$bld.c.egfr) <- "Clinical Blood - eGFR, recalculated"
  label(data$bld.c.egfraa) <- "Clinical Blood - eGFRAA, recalculated"
  label(data$bld.c.homair) <- "Clinical Blood - HOMA-IR (m/dL), recalculated"
  label(data$insulin.resist) <- "Clinical Blood - Insulin resistance, recalculated"
  label(data$bld.c.igg) <- "Clinical Blood-APA-IgG, set to 80.1 if `>80.0'"

  units(data$bld.c.insulin) <- "mcU/mL"
  units(data$bld.c.glucose) <- "mg/dL"
  units(data$bld.c.sodium) <- "mmol/L"
  units(data$bld.c.chloride) <- "mmol/L"
  units(data$bld.c.co2) <- "mmol/L"
  units(data$bld.c.urea.nitrogen) <- "mg/dL"
  units(data$bld.c.calcium) <- "mg/dL"
  units(data$bld.c.protein.t) <- "g/dL"
  units(data$bld.c.bilirubin.t) <- "mg/dL"
  units(data$bld.c.ast) <- "U/L"
  units(data$bld.c.alt) <- "U/L"
  units(data$bld.c.tsh) <- "mcU/mL"
  units(data$bld.c.creatinine) <- "mg/dL"
  units(data$bld.c.alkphos) <- "U/L"
  units(data$bld.c.crp) <- "mg/L"
  units(data$bld.c.albumin) <- "g/dL"
  units(data$bld.c.egfr) <- "mL/min"
  units(data$bld.c.egfraa) <- "mL/min"
  units(data$bld.c.homair) <- "m/dL"
  units(data$bld.c.igg) <- "g/L"
