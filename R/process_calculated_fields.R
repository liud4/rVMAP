process_calculated_fields <- function(data, data_label, epoch = current_epoch) {
  ...
  if (!grepl("\\.static$", data_label)) {
    ...
    if (!is.null(metadata.df)) {
      ...
      for (redcap_calculated_variables.i in dim(metadata.df)[1]) {
        element_variables <- pull(metadata.df[redcap_calculated_variables.i, "element_variables"])[[1]]
        for (row.i in dim(current_data.df)[1]) {
          current_data.df[row.i, current.var] <- ifelse(
            any(is.na(current_data.df[row.i, element_variables])),
            NA,
            current_data.df[row.i, current.var]
          )
        }
      }
    }
