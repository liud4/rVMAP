...
We begin by merging a subset of data sets together within each available epoch. The `merge_within_epoch()` function tidies each inputted data set, formats certain variables, sets some VMAC-specified values to missing, and merges them into a single data frame.
...
Now that we have created a single data frame for each available epoch, we will investigate any differences between the epochs and stack the four data frames into one longitudinal data set.
...
names(epoch1ToRepeat) <- c(Cs(map.id, diagnosis.factor.base), 
                           paste0(c(enrollmentVarsThatShouldNotChange, otherKeyEnrollmentVars), ".base"))
