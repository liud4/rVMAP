...
author: '[Panpan Zhang, Ph.D.](mailto:panpan.zhang@vumc.org)'
...
## base directories
onedrive.dir  <- file.path("..", "OneDrive - VUMC")
...
download.data <- FALSE
data.freeze.date <- 20250401 # in {yyyymmdd} format
# modify both of these variables to specify a specific data merge file to compare the new one with
last.data.download.date <- 20241210 # in {yyyymmdd} format
last.data.merge.date <- 20241210 # in {yyyymmdd} format

np_invalidation.file <- file.path(data.dir, "Documentation", "InvalidNeuropsych", "VMAP_NP_Invalids_Variable_Names_2024-12-03.xlsx")
map_163_invalidation.file <- file.path(data.dir, "Documentation", "MAP163_variablesunusable.xlsx")
composite.df <- readRDS(file.path(data.raw.dir, "np composite", "np_composite_20250401.rds"))
####                                                    ####
##%######################################################%##
...
data.merge.fm.file <- file.path(
  data.merge.dir, 
  paste0("MAP_fm_f", data.freeze.date, "_m", data.merge.date)
)
data.merge.bh.file <- file.path(
  data.merge.dir, 
  paste0("MAP_bh_f", data.freeze.date, "_m", data.merge.date)
)
...
if (!is.null(last.data.download.date) & is.null(last.data.merge.date)) {
  ...
  last.data.merge.suffix <- file.info(
    list.files(
      path = data.merge.dir,
      pattern = "MAP\\_bh\\_f\\d{8}\\_m\\d{8}.rds",
      full.names = T
    )
  ) %>%
    tibble::rownames_to_column(., var = "path") %>%
    ...
    filter(row_number() == 1) %>%
    pull(1) %>%
    stringr::str_extract("(?<=(MAP\\_bh\\_))f\\d{8}\\_m\\d{8}")
} else {
  last.data.merge.suffix <- paste0("f", last.data.download.date, "_m", last.data.merge.date)
}
...
# variables used for merging with other neuroimaging lists
brain_mri.variables <- c("map_id", "epoch", "session_id", "scanner", "scanner_software", "head_coil", "scan_date_time", grep("_scan_acquired", names(pre_vmap_edc_repeating.df), v = T))
...
brain_mri.metadata <- MAPfreeze.list$vmap_edc$metadata %>% 
  filter(
    field_name %in% c("session_id", "scanner", "scanner_software", "head_coil", "scan_date_time")
  )
# including ``scan_date_time`` for variable creation, too
...
pre_vmap_edc_scanner.df <- pre_vmap_edc_brain_mri.df %>% # data frame with repeating instruments limited to select scanner variables only
  select(
    c("session_id", "scanner", "scanner_software", "head_coil", "scan_date_time")
  ) %>% filter(
    !is.na(session_id)
  ) %>%
  mutate(scan_date = as.Date(scan_date_time)) %>%
  process_factor_variables(data = ., metadata = brain_mri.metadata) %>%
  select(
    session_id, scanner_factor, head_coil_factor, scanner_software_factor, scan_date
  ) %>%
  mutate(
...
form_names <- form_names[-grep("pet", form_names)] # ignore pet for temporarily
...
create_individual_form_df <- function(form) {
  ...
  form.var <- form.var[-grep("_scan_date", form.var)]
  ## not grab the ``_scan_date`` variables from the RedCap database
...
### Check for duplicated map_id and epoch in each neuroimaging form

```{r}
check_duplicate_neuroimaging <- function(mydat, myname){
  temp <- mydat %>% count(map_id, epoch) %>% filter(n > 1)
  if (nrow(temp) > 0){
    return(list("scanner (duplicates)" = myname, 
              "duplicates" = temp)
    )
  } else {NULL}
}

neuroimaging_duplicate.list <- lapply(
  seq_along(neuroimaging.list), 
  function(i) {
    check_duplicate_neuroimaging(neuroimaging.list[[i]], names(neuroimaging.list)[i])
  }
)

if (all(sapply(neuroimaging_duplicate.list, function(x) is.null(x)))) {
  cat("No duplicates found.\n\n")
} else {
  cat("Check list for duplicates.\n\n")
}
```

## Merge together selected brain MRI data to create a master VMAP EDC data set
...
## Merge together other brain MRI data to complete a master VMAP EDC data set

```{r}
# variables used for merging with other neuroimaging lists
add_brain_mri.variables <-
  unique(
    c(
      "map_id",
      "epoch",
      "session_id",
      grep("tone_", names(pre_vmap_edc_repeating.df), v = T),
      grep("flair_", names(pre_vmap_edc_repeating.df), v = T),
      grep("trust_", names(pre_vmap_edc_repeating.df), v = T),
      grep("dti_", names(pre_vmap_edc_repeating.df), v = T),
      grep("bo_", names(pre_vmap_edc_repeating.df), v = T),
      grep("vwi_", names(pre_vmap_edc_repeating.df), v = T),
      grep("neck_", names(pre_vmap_edc_repeating.df), v = T),
      grep("swi_", names(pre_vmap_edc_repeating.df), v = T),
      grep("pca_", names(pre_vmap_edc_repeating.df), v = T),
      grep("mo_", names(pre_vmap_edc_repeating.df), v = T),
      grep("asl_", names(pre_vmap_edc_repeating.df), v = T),
      grep("fmri_", names(pre_vmap_edc_repeating.df), v = T)
    )
  )

add_brain_mri.variables <-
  add_brain_mri.variables[add_brain_mri.variables %nin% grep("_scan_acquired", names(pre_vmap_edc_repeating.df), v = T)]

add_brain_mri.variables <-
  add_brain_mri.variables[add_brain_mri.variables %nin% grep("_session_id", names(pre_vmap_edc_repeating.df), v = T)]

add_brain_mri.variables <-
  add_brain_mri.variables[add_brain_mri.variables %nin% grep("_session_indicator", names(pre_vmap_edc_repeating.df), v = T)]


add_vmap_edc_brain_mri.df <- pre_vmap_edc_repeating.df %>%
  filter(redcap_repeat_instrument == "brain_mri") %>%
  select(all_of(add_brain_mri.variables))

tempfun <- function(add.var.list, pref.session) {
  current.session.id <-
    as_vector(vmap_edc_temp01.df %>% select(all_of(pref.session)))
  temp.df <-
    add_vmap_edc_brain_mri.df %>% filter(session_id %in% current.session.id) %>% select("map_id", "epoch", "session_id", all_of(add.var.list)) %>% as.data.frame()
  colnames(temp.df)[3] <- pref.session
  return(left_join(
    vmap_edc_temp01.df %>% select(!all_of(add.var.list)),
    temp.df,
    by = c("map_id", "epoch", pref.session)
  ))
}

# T1/DLMUSE
T1.brain.mri.var <-
  grep("tone_", names(add_vmap_edc_brain_mri.df), v = T)
T1.brain.mri.var <- T1.brain.mri.var[T1.brain.mri.var %nin% form_variables.df$variable]
vmap_edc_temp01.df <- tempfun(T1.brain.mri.var, "ma_session_id")

# white matter
wml.brain.mri.var <-
  grep("flair_", names(add_vmap_edc_brain_mri.df), v = T)
wml.brain.mri.var <- wml.brain.mri.var[wml.brain.mri.var %nin% form_variables.df$variable]
vmap_edc_temp01.df <- tempfun(wml.brain.mri.var, "wml_session_id")

# trust
trust.brain.mri.var <-
  grep("trust_", names(add_vmap_edc_brain_mri.df), v = T)
trust.brain.mri.var <- trust.brain.mri.var[trust.brain.mri.var %nin% form_variables.df$variable]
vmap_edc_temp01.df <- tempfun(trust.brain.mri.var, "oef_session_id")

# DTI
dti.brain.mri.var <-
  grep(("dti_"), names(add_vmap_edc_brain_mri.df), v = T)
bo.brain.mri.var <-
  grep(("bo_"), names(add_vmap_edc_brain_mri.df), v = T)
dti.brain.mri.var <- c(dti.brain.mri.var, bo.brain.mri.var)
dti.brain.mri.var <- dti.brain.mri.var[dti.brain.mri.var %nin% form_variables.df$variable]
vmap_edc_temp01.df <- tempfun(dti.brain.mri.var, "dti_session_id")

# VWI
vwi.brain.mri.var <-
  grep("vwi_", names(add_vmap_edc_brain_mri.df), v = T)
vwi.brain.mri.var <- vwi.brain.mri.var[vwi.brain.mri.var %nin% form_variables.df$variable]
vmap_edc_temp01.df <- tempfun(vwi.brain.mri.var, "vwi_session_id")

# neck
neck.brain.mri.var <-
  grep("neck_", names(add_vmap_edc_brain_mri.df), v = T)
neck.brain.mri.var <- neck.brain.mri.var[neck.brain.mri.var %nin% form_variables.df$variable]
vmap_edc_temp01.df <- tempfun(neck.brain.mri.var, "neck_session_id")

# SWI
swi.brain.mri.var <-
  grep("swi_", names(add_vmap_edc_brain_mri.df), v = T)
swi.brain.mri.var <- swi.brain.mri.var[swi.brain.mri.var %nin% form_variables.df$variable]
vmap_edc_temp01.df <- tempfun(swi.brain.mri.var, "swi_session_id")

# PCA
pca.brain.mri.var <-
  grep("pca_", names(add_vmap_edc_brain_mri.df), v = T)
pca.brain.mri.var <- pca.brain.mri.var[pca.brain.mri.var %nin% form_variables.df$variable]
vmap_edc_temp01.df <- tempfun(pca.brain.mri.var, "cow_session_id")

# ASL
mo.brain.mri.var <-
  grep("mo_", names(add_vmap_edc_brain_mri.df), v = T)
asl.brain.mri.var <-
  grep("asl_", names(add_vmap_edc_brain_mri.df), v = T)
asl.brain.mri.var <-
  asl.brain.mri.var[asl.brain.mri.var %nin% grep("_trust_", asl.brain.mri.var, v = T)]
asl.brain.mri.var <-
  asl.brain.mri.var[asl.brain.mri.var %nin% grep("_bold_", asl.brain.mri.var, v = T)]
asl.brain.mri.var <- c(asl.brain.mri.var, mo.brain.mri.var)
asl.brain.mri.var <- asl.brain.mri.var[asl.brain.mri.var %nin% form_variables.df$variable]
vmap_edc_temp01.df <- tempfun(asl.brain.mri.var, "asl_session_id")

# fMRI
fmri.brain.mri.var <-
  grep("fmri_", names(add_vmap_edc_brain_mri.df), v = T)
asl.bold.brain.mri.var <-
  grep("asl_", names(add_vmap_edc_brain_mri.df), v = T)
asl.bold.brain.mri.var <-
  asl.bold.brain.mri.var[asl.bold.brain.mri.var %in% grep("_bold_", asl.bold.brain.mri.var, v = T)]
fmri.brain.mri.var <- c(fmri.brain.mri.var, asl.bold.brain.mri.var)
fmri.brain.mri.var <- fmri.brain.mri.var[fmri.brain.mri.var %nin% form_variables.df$variable]

add_vmap_edc_brain_mri_fmri.df <- pre_vmap_edc_repeating.df %>%
  filter(redcap_repeat_instrument == "brain_mri") %>%
  select(c(
    "map_id",
    "epoch",
    "fmri_scan_acquired",
    all_of(fmri.brain.mri.var)
  )) %>%
  filter(fmri_scan_acquired == 1 &
           fmri_challenge_complete == 1 & fmri_scan_usable != 0) %>%
  group_by(map_id, epoch) %>%
  slice(1) %>%
  ungroup() %>%
  as.data.frame()
...
vmap_edc_temp01.df <-
  left_join(
    vmap_edc_temp01.df %>% select(!c(
      "fmri_scan_acquired", all_of(fmri.brain.mri.var)
    )),
    add_vmap_edc_brain_mri_fmri.df,
    by = c("map_id", "epoch")
  )

# vmap_edc_temp01.df %>% count(map_id, epoch) %>% filter(n > 1)
```
...
```{r}
process_nonscanner_factor_variables <- function(data, metadata) {
  ...
  if (!is.null(metadata.df)) {
    for (factor_var.i in metadata.df$field_name) {
      ...
      factor_var.i_levels_temp01 <- metadata.df[metadata.df$field_name == factor_var.i, "select_choices_or_calculations"]
      
      factor_var.i_levels_temp02 <- y <- gsub(factor_var.i_levels_temp01, pattern = "\\|", replacement = "| ") 
      
      factor_var.i_levels_temp03 <- y <- gsub(factor_var.i_levels_temp02, pattern = "\\|  ", replacement = "| ")
      
      factor_var.i_levels <- as.data.frame(factor_var.i_levels_temp03)
      
      levels.list <- as.list(REDCapR::checkbox_choices(pull(factor_var.i_levels)))
...
vmap_edc_temp02.df <- process_nonscanner_factor_variables(data = vmap_edc_temp01.df, metadata = MAPfreeze.list$vmap_edc$metadata)

###

vmap_edc_temp03.df <- vmap_edc_temp02.df %>%
  mutate(
    across(
      .cols = where(is.numeric),
      .fns = ~ replace(.x, .x %in% c(-1111*(6:9)), NA)
    ),
    across(
      .cols = where(is.character),
      .fns = ~ replace(.x, .x %in% c("-6666", "-7777", "-8888", "-9999", "1111-11-11", "11/11/1111"), NA)
    ),
    across(
...
vars_w_comparison_operators.var <- grep(
  ...
  grep(
    "notes|flow|occup",
    names(vmap_edc_temp03.df)[grepl("(>|<)([[:space:]]*)(\\d+)", vmap_edc_temp03.df)],
    invert = T,
    v = T),
  v = T)
...
if (length(vars_w_comparison_operators.var) > 0) {
  for (var in vars_w_comparison_operators.var) {
    vmap_edc_temp03.df[, var] <- suppressWarnings(sapply(vmap_edc_temp03.df[, var], process_comparison_operators, USE.NAMES = FALSE))
  }
}

###

# improperly_formatted_dttm.var <-
#   grep("date_time$", names(vmap_edc_temp03.df), v = T)[sapply(vmap_edc_temp03.df[, grep("date_time$", names(vmap_edc_temp03.df), v = T)], is.character)]
# 
...
vmap_edc_temp04.df <-
  vmap_edc_temp04.df %>% mutate(pet_consent_date = lubridate::as_date(pet_consent_date)) %>% as.data.frame() # pet_consent_date in fact is a "date + time" variable.
```
...
**Note:** The `physlog.static` has retired from MAP data merge.
...
```{r}
vmap_edc.df <- vmap_edc_temp05.df
...
discard_medication.var <- grep("name$|units$", grep("med\\d{2}", names(questionnaires_temp04.df), v = T), v = T, invert = T)
...
questionnaires_temp06a_medname.df <- questionnaires_temp05.df %>% 
  filter(!is.na(redcap_repeat_instrument)) %>% 
  select(
    map_id, vmac_id, epoch, matches("med\\d+_name$")
  )

questionnaires_temp06b_medname.df <- questionnaires_temp06a_medname.df %>%
  group_by(vmac_id, epoch) %>%
  # arrange(pick(starts_with("med")), .by_group = TRUE) %>%
  mutate(
...
questionnaires_temp06b_medname.df$redcap_repeat_instrument <- NULL

###

questionnaires_temp06a_medunits.df <- questionnaires_temp05.df %>% 
  filter(!is.na(redcap_repeat_instrument)) %>% 
  select(
    map_id, vmac_id, epoch, matches("med\\d+_units$")
  )

questionnaires_temp06b_medunits.df <- questionnaires_temp06a_medunits.df %>%
  group_by(vmac_id, epoch) %>%
  # arrange(pick(starts_with("med")), .by_group = TRUE) %>%
  mutate(
    temp_suffix = row_number(),
  ...
  tidyr::pivot_wider(
    names_from = temp_suffix,
    values_from = matches("med\\d+_units$"),
    names_glue = "med{formatC(as.numeric(gsub('med|units|_', '', .value)) + (15 * (as.numeric(temp_suffix) - 1)), width = 2, flag = '0')}_units"
  ) %>%
  select(
    order(colnames(.))
  ) %>%
...
questionnaires_temp06b_medunits.df$redcap_repeat_instrument <- NULL

###

questionnaires_temp06a_surgnum.df <- questionnaires_temp05.df %>% 
  filter(!is.na(redcap_repeat_instrument)) %>% 
  select(
    map_id, vmac_id, epoch, matches("surg\\d+$")
  ) %>%
  select(
    -surg16
  )

questionnaires_temp06b_surgnum.df <- questionnaires_temp06a_surgnum.df %>%
  group_by(vmac_id, epoch) %>%
  arrange(pick(starts_with("surg")), .by_group = TRUE) %>%
  mutate(
    temp_suffix = row_number(),
    .before = "map_id"
  ) %>%
  tidyr::pivot_wider(
    names_from = temp_suffix,
    values_from = matches("surg\\d+$"),
    names_glue = "surg{formatC(as.numeric(gsub('surg', '', .value)) + (15 * (as.numeric(temp_suffix) - 1)), width = 2, flag = '0')}"
  ) %>%
  select(
    order(colnames(.))
  ) %>%
...
questionnaires_temp06b_surgnum.df$redcap_repeat_instrument <- NULL

###

questionnaires_temp06a_surgname.df <- questionnaires_temp05.df %>% 
  filter(!is.na(redcap_repeat_instrument)) %>% 
  select(
    map_id, vmac_id, epoch, matches("surg\\d+_name$")
  )

questionnaires_temp06b_surgname.df <- questionnaires_temp06a_surgname.df %>%
  group_by(vmac_id, epoch) %>%
  arrange(pick(starts_with("surg")), .by_group = TRUE) %>%
  mutate(
    temp_suffix = row_number(),
  ...
  tidyr::pivot_wider(
    names_from = temp_suffix,
    values_from = matches("surg\\d+_name$"),
    names_glue = "surg{formatC(as.numeric(gsub('surg|name|_', '', .value)) + (15 * (as.numeric(temp_suffix) - 1)), width = 2, flag = '0')}_name"
  ) %>%
  select(
    order(colnames(.))
  ) %>%
...
questionnaires_temp06b_surgname.df$redcap_repeat_instrument <- NULL

###

questionnaires_temp06a_surgyear.df <- questionnaires_temp05.df %>% 
  filter(!is.na(redcap_repeat_instrument)) %>% 
  select(
    map_id, vmac_id, epoch, matches("surg\\d+_year$")
  )

questionnaires_temp06b_surgyear.df <- questionnaires_temp06a_surgyear.df %>%
  group_by(vmac_id, epoch) %>%
  arrange(pick(starts_with("surg")), .by_group = TRUE) %>%
  mutate(
    temp_suffix = row_number(),
  ...
  tidyr::pivot_wider(
    names_from = temp_suffix,
    values_from = matches("surg\\d+_year$"),
    names_glue = "surg{formatC(as.numeric(gsub('surg|year|_', '', .value)) + (15 * (as.numeric(temp_suffix) - 1)), width = 2, flag = '0')}_year"
  ) %>%
  select(
    order(colnames(.))
  ) %>%
...
questionnaires_temp06b_surgyear.df$redcap_repeat_instrument <- NULL
...
questionnaires_temp07.df <- questionnaires_temp05.df %>% 
  ...
  select(!matches("med\\d+_units$")) %>%
  select(!matches("surg\\d+$")) %>%
  select(!matches("surg\\d+_name$")) %>%
  select(!matches("surg\\d+_year$"))
...
questionnaires_temp07.df <- left_join(questionnaires_temp07.df, questionnaires_temp06b_medname.df, by = c("vmac_id", "map_id", "epoch")) %>%
  left_join(., questionnaires_temp06b_medunits.df, by = c("vmac_id", "map_id", "epoch")) %>%
  left_join(., questionnaires_temp06b_surgnum.df, by = c("vmac_id", "map_id", "epoch")) %>%
  left_join(., questionnaires_temp06b_surgname.df, by = c("vmac_id", "map_id", "epoch")) %>%
  left_join(., questionnaires_temp06b_surgyear.df, by = c("vmac_id", "map_id", "epoch")) %>%
  arrange(
    map_id, epoch
  )

# questionnaires_temp07.df %>% group_by(map_id) %>% filter(n() == 1) %>% select(map_id, epoch) %>% ungroup() # these participants only have eligibility data
# questionnaires_temp07.df %>% count(map_id, epoch) %>% filter(n > 1) # check to see that there is only one row per participant per epoch
```
...
extra_vars_to_pull_down_from_eligibility.var <- MAPfreeze.list$questionnaires$metadata %>%
  filter(
    form_name %in% extra_forms_to_pull_down_from_eligibility.var
  ) %>%
  pull(
...
extra_vars_to_pull_down_from_eligibility.var <- extra_vars_to_pull_down_from_eligibility.var[extra_vars_to_pull_down_from_eligibility.var %in% names(questionnaires_temp07.df)]

questionnaires_temp07.df[questionnaires_temp07.df$epoch %in% c(0, 1) & questionnaires_temp07.df$map_id >= 337, c("map_id", "epoch", extra_vars_to_pull_down_from_eligibility.var)] <- questionnaires_temp07.df[questionnaires_temp07.df$epoch %in% c(0, 1) & questionnaires_temp07.df$map_id >= 337, c("map_id", "epoch", extra_vars_to_pull_down_from_eligibility.var)] %>%
  group_by(
    map_id
  ) %>%
  tidyr::fill(
...
## Temporary codes: carrying over epoch 0 "gds" and "qids" scores to epoch 1
for (var.i in extra_vars_to_pull_down_from_eligibility.var) {
  map_id_selected <-
    unlist(questionnaires_temp07.df[questionnaires_temp07.df$map_id >= 337 &
                                      questionnaires_temp07.df$epoch == 1, "map_id"]) %>% as.numeric()
  questionnaires_temp07.df[questionnaires_temp07.df$epoch == 1 &
                             questionnaires_temp07.df$map_id  %in% map_id_selected, var.i] <-
    questionnaires_temp07.df[questionnaires_temp07.df$epoch == 0 &
                               questionnaires_temp07.df$map_id %in% map_id_selected, var.i]
} 
```

```{r}
questionnaires_temp08.df <- questionnaires_temp07.df

forms_to_exclude.var <- c("consent_participant", "medications_review_form", "consent_study_partner", "surgical_review", "medications", "health_history", "surgical_history")

exclude_vars_from_forms.var <- MAPfreeze.list$questionnaires$metadata %>%
  filter(
    form_name %in% forms_to_exclude.var
  ) %>%
  pull(
    field_name
  )

exclude_vars_from_forms.var <- c(exclude_vars_from_forms.var, paste0(exclude_vars_from_forms.var, "_factor"))

exclude_vars_from_forms.var <- exclude_vars_from_forms.var[exclude_vars_from_forms.var %in% names(questionnaires_temp08.df)]

vars_not_to_pull_down.var <- unique(c("vmac_id", "redcap_event_name", "map_id", "epoch", exclude_vars_from_forms.var, extra_vars_to_pull_down_from_eligibility.var))

questionnaires_temp08.df[questionnaires_temp08.df$epoch %in% c(0, 1), ] <- questionnaires_temp08.df[questionnaires_temp08.df$epoch %in% c(0, 1), ] %>%
  group_by(
    map_id
  ) %>%
  tidyr::fill(
    setdiff(names(questionnaires_temp08.df), vars_not_to_pull_down.var), .direction = "down"
  ) %>%
  ungroup()
```
...
invalid_np_DL.var <- readxl::read_xlsx(np_invalidation.file, col_names = TRUE)
invalid_np.temp <- merged.df %>% select(c(map_id, epoch, names(invalid_np_DL.var))) %>% as.data.frame()
invalid_np.list <- lapply(1:ncol(invalid_np_DL.var), function(x) {
  temp.var <- names(invalid_np_DL.var)[x]
  temp <- invalid_np.temp[c("map_id", "epoch", temp.var)]
  temp[temp[temp.var] == 1, ] %>% filter(!is.na(map_id))
})

invalid_np_DL.df <- NULL
for (i in 1:ncol(invalid_np_DL.var)){
  temp <- do.call("rbind", replicate(nrow(na.omit(invalid_np_DL.var[, i])), invalid_np.list[[i]], simplify = FALSE))
  temp.df <- cbind(temp$map_id, temp$epoch, rep(na.omit(as.matrix(invalid_np_DL.var)[, i]), each = nrow(invalid_np.list[[i]])))
  invalid_np_DL.df <- rbind(invalid_np_DL.df, temp.df)
}

invalid_np_DL.df <- as.data.frame(invalid_np_DL.df)
names(invalid_np_DL.df) <- c("map_id", "epoch", "test")
invalid_np_DL.df %>% mutate(epoch = as.integer(epoch)) %>% as.data.frame()

for (row.i in 1:nrow(invalid_np_DL.df)) {
  invalidation.df <- invalid_np_DL.df[row.i, ]
  merged.df[merged.df$map_id == invalidation.df$map_id & merged.df$epoch == invalidation.df$epoch, invalidation.df$test] <- NA
}
```
...
merged.df <- rVMAP::derive_demographics(merged.df)
```
...
```{r derive-AD-sig}
merged.df <- derive_AD_signature_temp(merged.df)
class(merged.df) <- "data.frame"
```
...
**These scores were derived by Dandan Liu on June 08, 2023 in MPlus.**
...
```{r biospecimen}
merged.df.bkup <- merged.df
merged.df <- process_biospecimen_availability(merged.df)
```
...
# Summary of Essential Missing Demographics

```{r}
missing.all.df <- merged.df %>% select(c(map.id, epoch, age, ethnicity.factor, education, fsrp.minus.age.points)) %>% filter(is.na(age) | is.na(ethnicity.factor) | is.na(education) | is.na(fsrp.minus.age.points))
missing.all.df
```

## Summary of Essential Missing Demographics (at Baseline)

```{r}
missing.bl.df <- missing.all.df %>% filter(epoch == 1)
missing.bl.df
```

## Investigation of Missing Baseline Ages

```{r}
id_age <- missing.bl.df %>% filter(is.na(age)) %>% select(map.id)
merged.df %>% select(c(map.id, epoch, vf.arrival.date.time, medhx.date, dob)) %>% filter(map.id %in% id_age$map.id & epoch == 1)
```

## Investigation of Missing Baseline Ethnicity

```{r}
id_ethnicity <- missing.bl.df %>% filter(is.na(ethnicity.factor)) %>% select(map.id)
merged.df %>% select(c(map.id, epoch, race, ethnicity)) %>% filter(map.id %in% id_ethnicity$map.id & epoch == 1)
```

## Investigation of Missing Baseline Education

```{r}
id_education <- missing.bl.df %>% filter(is.na(education)) %>% select(map.id)
merged.df %>% select(c(map.id, epoch, education)) %>% filter(map.id %in% id_education$map.id & epoch == 1)
```

## Investigation of Missing Baseline FSRP (Minus Age Points)

```{r}
id_fsrp_minus_age <-
  missing.bl.df %>% filter(is.na(fsrp.minus.age.points)) %>% select(map.id)
merged.df %>% select(
  c(
    map.id,
    epoch,
...
) %>% filter(map.id %in% id_fsrp_minus_age$map.id & epoch == 1)
```

### Further Investigations (Participant by Participant)

```{r}
merged.df %>% filter(map.id == "127" & epoch == 1) %>% select(c(map.id, epoch, echo.lvidd, echo.lvpwd, echo.ivsd, bsa))

merged.df %>% filter(map.id == "373" & epoch == 1) %>% select(c(map.id, epoch, bld.c.hgba1c, diabetesrx, bld.c.glucose))

merged.df %>% filter(map.id == "450" & epoch == 1) %>% select(c(map.id, epoch, bld.c.hgba1c, diabetesrx, bld.c.glucose))

merged.df %>% filter(map.id == "458" & epoch == 1) %>% select(c(map.id, epoch, bld.c.hgba1c, diabetesrx, bld.c.glucose))

merged.df %>% filter(map.id == "466" & epoch == 1) %>% select(c(map.id, epoch, bld.c.hgba1c, diabetesrx, bld.c.glucose))
```
...
saveRDS(merged.df, file = paste0(data.merge.dir, "/MAP_fm_", output_file_name, ".rds"))
write.csv(merged.df, file = paste0(data.merge.dir, "/MAP_fm_", output_file_name, ".csv"), row.names = FALSE)
```
