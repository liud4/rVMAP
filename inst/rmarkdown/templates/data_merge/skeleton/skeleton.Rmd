...
labels.0 <- lapply(MAPfreeze.list[["epoch_0"]][["metadata"]][c("eligibility", "apoe")], format_metadata) %>% bind_rows() %>% mutate(epoch = 0)
labels.1 <- lapply(MAPfreeze.list[["epoch_1"]][["metadata"]][c("main", "cardiac.mri", "man3T", "man3T.bh", "auto3T", "auto3T.bh", "csf", "biomarkers", "srt", "addendum")], format_metadata) %>% bind_rows() %>% mutate(epoch = 1)
labels.2 <- lapply(MAPfreeze.list[["epoch_2"]][["metadata"]], format_metadata) %>% bind_rows() %>% mutate(epoch = 2)
labels.3 <- lapply(MAPfreeze.list[["epoch_3"]][["metadata"]], format_metadata) %>% bind_rows() %>% mutate(epoch = 3)
labels.4 <- lapply(MAPfreeze.list[["epoch_4"]][["metadata"]], format_metadata) %>% bind_rows() %>% mutate(epoch = 4)
...
variables_with_contradicting_labels_on_REDCap.df <- labels.all[labels.all$field_name %in% duplicated.names, Cs(field_name, field_label, epoch)] %>% arrange(field_name, epoch)

saveRDS(variables_with_contradicting_labels_on_REDCap.df, 
        file = file.path(data.merge.dir, 
                         paste0("variables_with_contradicting_labels_on_REDCap",
                                "_d",
                                data.freeze.date,
                                "_m",
                                data.merge.date,
...
labels.df <- bind_rows(labels.all, labels.bHold.df, labels.factor.df)

for (var.i in labels.df$field_name) {
  Hmisc::label(merged.df[, var.i]) <- pull(labels.df[labels.df$field_name == var.i, "field_label"])
}
...
This section highlights changes between the current and previous breath hold data sets. For ease of reading the output, "notes"-type variables, "med"-type variables, etc. are not included in the comparison. For sake of simplicity, we will look only at variables that are common to both data sets. A CSV file for each epoch is saved by defualt to the `box/VMAC BIOSTAT/DATA/MAP/mergedData` directory.
...
if (is.null(last.data.merge.bh.file)) {
  ...
  common_var <- intersect(names(merged.bh.df), names(prev.bh.df))
  vars_to_delete <- c(grep("^med\\d+", common_var, v = T), grep("notes", common_var, v = T), grep("^entry", common_var, v = T), grep("famhx\\d+", common_var, v = T))
  common_var <- common_var[!common_var %in% vars_to_delete]
}
```
...
current.e1.df <- merged.bh.df %>%
  ...
  select(
    one_of(c('map.id', 'epoch', common_var))
  ) %>%
  select_if(~!all(is.na(.))) %>%
...
write.csv(e1.diff, file = file.path(data.merge.dir, paste0("e1_diff", current.data.merge.suffix, ".csv")), row.names = FALSE)

summary(e1.comparison)
```
...
write.csv(e2.diff, file = file.path(data.merge.dir, paste0("e2_diff", current.data.merge.suffix, ".csv")), row.names = FALSE)

summary(e2.comparison)
```
...
write.csv(e3.diff, file = file.path(data.merge.dir, paste0("e3_diff", current.data.merge.suffix, ".csv")), row.names = FALSE)

summary(e3.comparison)
```
...
write.csv(e4.diff, file = file.path(data.merge.dir, paste0("e4_diff", current.data.merge.suffix, ".csv")), row.names = FALSE)

summary(e4.comparison)
```
