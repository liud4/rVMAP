...
mem.composite.file <- file.path(data.dir, 'rawData', 'Memory_Scores_20190620.rds')
ef.composite.file <- file.path(data.dir, 'rawData', 'EF_Scores_20190620.rds')
...
merged.df[, Cs(mlta01, psqi_time_sleep, psqi_hrs_sleep, famhx01_age_ptp, famhx02_age_ptp, famhx01_age_inf, famhx02_age_inf, famhx03_age_inf, actigraph_day1_mins, actigraph_day2_mins, actigraph_day3_mins, actigraph_day4_mins, actigraph_day5_mins, actigraph_day6_mins, actigraph_day7_mins, actigraph_day8_mins, actigraph_day9_mins, actigraph_day10_mins, actigraph_day11_mins, echo_sbp2, echo_tissue_dop_e_med, echo_mv_dt, echo_stroke_vol_vti, cmr_findings_pulse, cmr_findings_dbp, cmr_lvedv, cmr_rvedd, cmr_ascending_ao_max_area, np_pvltrecog_falsepos, np_3digits_span, np_3digits_intrus, np_3digits_cap, np_3digits_frontal, np_3digits_errors, np_3digits_btcfor, np_3digits_btcback)] <- sapply(merged.df[, Cs(mlta01, psqi_time_sleep, psqi_hrs_sleep, famhx01_age_ptp, famhx02_age_ptp, famhx01_age_inf, famhx02_age_inf, famhx03_age_inf, actigraph_day1_mins, actigraph_day2_mins, actigraph_day3_mins, actigraph_day4_mins, actigraph_day5_mins, actigraph_day6_mins, actigraph_day7_mins, actigraph_day8_mins, actigraph_day9_mins, actigraph_day10_mins, actigraph_day11_mins, echo_sbp2, echo_tissue_dop_e_med, echo_mv_dt, echo_stroke_vol_vti, cmr_findings_pulse, cmr_findings_dbp, cmr_lvedv, cmr_rvedd, cmr_ascending_ao_max_area, np_pvltrecog_falsepos, np_3digits_span, np_3digits_intrus, np_3digits_cap, np_3digits_frontal, np_3digits_errors, np_3digits_btcfor, np_3digits_btcback)], as.numeric)
```
...
labels.1 <- lapply(MAPfreeze.list[["epoch_1"]][["metadata"]][c("main", "cardiac.mri", "man3T", "man3T.bh", "auto3T", "auto3T.bh", "csf", "biomarkers", "srt", "addendum")], format_metadata) %>% 
  bind_rows() %>% 
  mutate(epoch = 1)
...
labels.all <- dplyr::bind_rows(labels.elig, labels.apoe, labels.1, labels.2, labels.3, labels.4) %>% 
...
```{r repeatBaseline}
enrollment_vars <- Cs(  # will be used as is
  dob,
  education,
  sex.factor,
  apoe4pos.factor,
...
for (vname in baseline_vars) {
  label(to_repeat.df[[vname]]) <- paste0(label(to_repeat.df[[vname]]), ", from Enrollment")
}

names(to_repeat.df) <- c('map.id', enrollment_vars, paste0(baseline_vars, ".base"))

merged.df <- merged.df[, !(names(merged.df) %in% enrollment_vars)] %>% 
  left_join(
    to_repeat.df,
    by = "map.id"
  )
...
## Re-ordering Levels in `diagnosis.factor.base`
...
These scores were provided to us in two RDS files dated June 20, 2019 by Timothy Hohman after calculating them in MPlus. We also manually set `np.memory.composite` to missing for MAP ID 209 in Epoch 1.

```{r warning = FALSE, message = FALSE}
mem.composite.df <- readRDS(mem.composite.file) %>%
  rename(np.memory.composite = memory.composite)

ef.composite.df <- readRDS(ef.composite.file) %>%
