...
# Add Units Attribute to Each Variable

```{r}
units.all <- dplyr::bind_rows(labels.elig, labels.apoe, labels.1, labels.2, labels.3, labels.4, labels.5) %>% 
  dplyr::distinct(field_name, field_annotation, .keep_all = TRUE)

units.all %<>%
  group_by(field_name) %>%
  arrange(field_name, epoch) %>%
  select(field_name, field_annotation, epoch) %>%
  slice(1) %>%
  ungroup() %>%
  drop_na()

units.bHold.df <- units.all %>%
  filter(
    field_name %in% gsub("bHold.", "", names(merged.df)[names(merged.df) %in% paste0("bHold.", units.all$field_name)])
  ) %>%
  select(
    Cs(field_name, field_annotation, epoch)
  ) %>%
  mutate(
    field_name = paste0("bHold.", field_name)
  )

units.factor.df <- units.all %>%
  filter(
    field_name %in% gsub(".factor", "", names(merged.df)[names(merged.df) %in% paste0(units.all$field_name, ".factor")])
  ) %>%
  select(
    Cs(field_name, field_annotation, epoch)
  ) %>%
...
units.bHold.factor.df <- units.all %>%
  filter(
    field_name %in% gsub("bHold\\.|\\.factor", "", names(merged.df)[names(merged.df) %in% paste0("bHold.", units.all$field_name, ".factor")])
  ) %>%
  select(
    Cs(field_name, field_annotation, epoch)
  ) %>%
...
units.elig.df <- units.all %>%
  filter(
    field_name %in% gsub(".elig", "", names(merged.df)[names(merged.df) %in% paste0(units.all$field_name, ".elig")])
  ) %>%
  select(
    Cs(field_name, field_annotation, epoch)
  ) %>%
...
units.elig.factor.df <- units.all %>%
  filter(
    field_name %in% gsub(".factor.elig", "", names(merged.df)[names(merged.df) %in% paste0(units.all$field_name, ".factor.elig")])
  ) %>%
  select(
    Cs(field_name, field_annotation, epoch)
  ) %>%
...
units.df <- bind_rows(units.all, units.bHold.df, units.factor.df, units.bHold.factor.df, units.elig.df, units.elig.factor.df) %>%
  filter(
    field_name %in% names(merged.df)
  )
...
```{r derive-mltaChamps}
merged.df$mlta10 <- as.numeric(merged.df$mlta10)
...
merged.df$inform.length.yrs <- as.numeric(merged.df$inform.length.yrs)
merged.df$inform.length.mos <- as.numeric(merged.df$inform.length.mos)
...
These scores were provided to us in two RDS files dated March 29, 2021 by Timothy Hohman after calculating them in MPlus.
...
# Adding Neuroimaging Scanner Data

```{r neuroimaging-link}
merged.df <- call_function("process_neuroimaging_link", merged.df)
```

# Adding Biospecimen Availability

```{r biospecimen}
biospecimen.df <- process_biospecimen_availability(path = "~/box/VMAC BIOSTAT/DATA/MAP/rawData/VMAP_biospecimens_collected_20210719.xlsx", epoch = 1:5)

biospecimen.df$epoch <- as.numeric(biospecimen.df$epoch)
...
```{r ordering-alphabetically}
merged.bh.df <- merged.bh.df[, order(colnames(merged.bh.df))] %>%
  select(
    map.id, epoch, everything()
  )

merged.fm.df <- merged.fm.df[, order(colnames(merged.fm.df))] %>%
  select(
    map.id, epoch, everything()
  )
```
...
```{r}
tibble(new_variable = sort(current.all.var[which(current.all.var %nin% prev.all.var)]))
```
