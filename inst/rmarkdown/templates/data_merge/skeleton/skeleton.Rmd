...
# form_names <- c("freesurfer_2005_reconstats", "freesurfer_2009_reconstats", 
# "freesurfer_lobes", "freesurfer_hippocampal_subfields", "freesurfer_brain_volume",
# "multiatlas_stats", "lesion_segmentation_tool_stats", "vbm_qa_stats",
# "fsl_first_stats", "hippodeep", "white_matter_stamper_stats",
# "asl_multiatlas_stats", "dtiroi", "veasl_stats", "oxygen_extraction_fraction",
...
```{r}
physlog.df <- read_csv("~/Box Sync/VMAC BIOSTAT/DATA/MAP/rawData/20220916 - vmap_physlog_data.csv", show_col_types = FALSE) %>%
  select(
    -session_id
  ) %>%
  mutate(
...
questionnaires_temp04.df <- convert_dates(questionnaires_temp03.df) # will output warning -- see "Notes" section above
...
```{r}
questionnaires.df <- questionnaires_temp04.df
...
```{r}
# merged.df %>% tibble() %>% filter(map_id == "007") %>% select(epoch, np_tower1)

invalid_np.df <- readxl::read_excel("~/Box Sync/VMAC BIOSTAT/DATA/MAP/Documentation/InvalidNeuropsych/VMAP NP Invalids QC 8.29.22_DL.xlsx") %>%
  format_id() %>%
  filter(
    epoch != 0
  ) %>%
...
```{r biospecimen}
biospecimen.df <- process_biospecimen_availability(path = "~/Box Sync/VMAC BIOSTAT/DATA/MAP/rawData/VMAP_biospecimens_collected_20210719.xlsx", epoch = 1L:5L)
